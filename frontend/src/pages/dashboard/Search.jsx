  ...
  return (
    <div className="space-y-8">
      <div className="max-w-3xl mx-auto text-center space-y-4">
        ...
        <p className="text-slate-300">Find what you need across all your folders and files.</p>
        <div className="relative mt-8">
          <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" />
          <input 
            type="text" 
            placeholder="Search by title, category, or type..." 
            className="w-full h-14 pl-12 pr-4 rounded-2xl border-2 border-slate-200 white:border-slate-800 bg-white dark:bg-slate-900 focus:border-primary-500 focus:ring-0 transition-all outline-none text-lg"
            value={query}
      ...
      <div className="grid lg:grid-cols-4 gap-8">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 font-bold mb-4 text-white">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-2">
                <FilterCheckbox label="PDF Documents" count={documents.filter(d => d.type === 'PDF').length} />
                <FilterCheckbox label="Word Files" count={documents.filter(d => d.type === 'DOCX').length} />
                <FilterCheckbox label="Presentations" count={documents.filter(d => d.type === 'PPTX').length} />
                <FilterCheckbox label="Text Files" count={documents.filter(d => d.type === 'TXT').length} />
              </div>
            </div>

            <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
              <p className="text-sm font-semibold mb-2 uppercase tracking-wider text-white">Date Range</p>
              <div className="space-y-2">
        ...
        <div className="lg:col-span-3 space-y-6">
          {query.length > 0 ? (
            ...
            <div className="space-y-6">
              ...
              <div className="pt-8">
                <h2 className="text-lg font-bold mb-4 text-white">Quick Searches</h2>
                <div className="flex flex-wrap gap-2">
                  {['Reports', 'Strategy', 'Proposals', 'Finance'].map(s => (
